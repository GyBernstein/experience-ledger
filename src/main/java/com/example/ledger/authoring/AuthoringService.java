...
public class AuthoringService {
 private final Db db;private final LedgerService ledger;private final DraftAssistProvider provider;private final PromptService prompts;private final DraftSchema schema;
 public AuthoringService(Db db,LedgerService ledger,DraftAssistProvider provider,PromptService prompts,DraftSchema schema){this.db=db;this.ledger=ledger;this.provider=provider;this.prompts=prompts;this.schema=schema;}
 ...
 private Map<String,Object> capture(ActorContext actor,String channel,String agentRole,String raw,String sourceType,String sourceRef,JsonNode taskContext,List<String> evidenceRefs,String language,boolean generate,String dedupKey){
  ...
  var captured=ledger.capture(actor,new Requests.Capture(raw,"EXPERIENCE",sourceType,channel.equals("AGENT")?actor.actorId():"authoring-ui",sourceRef,"AUTHORING_CAPTURE",dedupKey,extracted,null,null));UUID candidate=(UUID)captured.get("id");
  db.with(actor,"store authoring context",()->{db.update("""
   ...
   """,db.scoped(actor,"id",candidate,"channel",channel,"role",agentRole,"context",db.stringify(object(taskContext)),"evidence",db.stringify(evidenceRefs==null?List.of():evidenceRefs),"language",blank(language)?"zh-CN":language,"hash",schema.hash(raw)));return null;});
 ...
 private Map<String,Object> generate(ActorContext actor,UUID candidate,String operation,String source,String instruction){
  var input=input(actor,candidate);var prompt=prompts.current(actor,"EXPERIENCE_DRAFT_V1");long started=System.nanoTime();DraftAssistProvider.Result result=null;
  try{
   ...
   try{normalized=schema.normalize(result.content(),(String)input.candidate().get("raw_content"),input.evidenceIds());}
   catch(LedgerException invalid){
    var repairPrompt=prompts.current(actor,"EXPERIENCE_REVISION_V1");result=provider.revise(new DraftAssistProvider.RevisionRequest(actor,(String)input.candidate().get("raw_content"),input.context(),input.evidence(),input.language(),result.content(),"修复 JSON Schema："+invalid.getMessage()+"。不得补造事实。",repairPrompt));
    normalized=schema.normalize(result.content(),(String)input.candidate().get("raw_content"),input.evidenceIds());prompt=repairPrompt;
   }
 ...
 public Map<String,Object> revise(ActorContext actor,UUID draftId,AuthoringRequests.Revision r){actor.requireGovernance();var current=current(actor,draftId,r.expectedDraftVersion());var input=input(actor,(UUID)current.get("candidate_id"));var prompt=prompts.current(actor,"EXPERIENCE_REVISION_V1");long started=System.nanoTime();
  try{var result=provider.revise(new DraftAssistProvider.RevisionRequest(actor,(String)input.candidate().get("raw_content"),input.context(),input.evidence(),input.language(),(JsonNode)current.get("structured_content_json"),r.instruction(),prompt));var normalized=schema.normalize(result.content(),(String)input.candidate().get("raw_content"),input.evidenceIds());return insertDraft(actor,input,normalized,result,prompt,"REVISE","AI_REVISED",r.instruction(),started,null);}catch(RuntimeException e){throw e;}
 }

 public Map<String,Object> edit(ActorContext actor,UUID draftId,AuthoringRequests.ManualEdit r){actor.requireGovernance();var current=current(actor,draftId,r.expectedDraftVersion());var input=input(actor,(UUID)current.get("candidate_id"));ObjectNode normalized=schema.normalize(r.structuredContent(),(String)input.candidate().get("raw_content"),input.evidenceIds());preserveAgentOrigins((JsonNode)current.get("structured_content_json"),normalized,r.structuredContent());
  var result=new DraftAssistProvider.Result(normalized,"human","manual-edit",null,null,0d,"USD");var prompt=prompts.current(actor,"EXPERIENCE_REVISION_V1");return insertDraft(actor,input,normalized,result,prompt,"REVISE","HUMAN_EDITED",r.reason(),System.nanoTime(),null);
 }
 ...
 public Map<String,Object> accept(ActorContext actor,UUID draftId,AuthoringRequests.Publication r){actor.requireGovernance();var existing=publication(actor,draftId);if(existing!=null)return existing;
  var row=current(actor,draftId,r.expectedDraftVersion());UUID candidateId=(UUID)row.get("candidate_id");JsonNode doc=(JsonNode)row.get("structured_content_json");List<Requests.ClaimInput> claims=publicationClaims(actor,doc);
  ...
  var candidate=ledger.candidate(actor,candidateId);var reviewed=ledger.review(actor,candidateId,new Requests.Review(((Number)candidate.get("revision")).intValue(),draft,r.reason()));
  ...
  return Map.of("draftId",draftId,"candidateId",candidateId,"experience",version,"compact",Map.of("l1",schema.compact(doc),"l2",schema.summary(doc)));
 }
 ...
 private Input input(ActorContext actor,UUID candidate){return db.with(actor,null,()->{var c=db.one("select * from exp_candidate where space_id=:space and id=:id",db.scoped(actor,"id",candidate));var meta=db.one("select * from exp_authoring_candidate where space_id=:space and candidate_id=:id",db.scoped(actor,"id",candidate));JsonNode refs=(JsonNode)meta.get("evidence_refs_json");List<DraftAssistProvider.EvidenceRef> evidence=new ArrayList<>();Set<String> ids=new LinkedHashSet<>();if(refs!=null)for(JsonNode ref:refs){String value=ref.asText();List<Map<String,Object>> found=db.list("select id,evidence_type,source_system,source_ref,reliability from exp_evidence where space_id=:space and (case when :isUuid then id=cast(:ref as uuid) else source_ref=:ref end) and status='ACTIVE' limit 1",db.scoped(actor,"isUuid",uuid(value),"ref",value));if(!found.isEmpty()){var e=found.getFirst();String id=e.get("id").toString();ids.add(id);evidence.add(new DraftAssistProvider.EvidenceRef(id,(String)e.get("evidence_type"),(String)e.get("source_system"),(String)e.get("source_ref"),((Number)e.get("reliability")).doubleValue()));}}
  return new Input(c,(JsonNode)meta.get("task_context_json"),evidence,ids,(String)meta.get("language"));});}
 private record Input(Map<String,Object> candidate,JsonNode context,List<DraftAssistProvider.EvidenceRef> evidence,Set<String> evidenceIds,String language){}
 ...
 private List<Map<String,Object>> similar(ActorContext actor,JsonNode d){String title=d.path("title").asText();if(title.isBlank())return List.of();return db.list("select f.id family_id,v.id version_id,v.title,v.summary,f.domain from exp_experience_version v join exp_experience_family f on f.space_id=v.space_id and f.id=v.family_id where v.space_id=:space and v.status='VERIFIED' and v.search_vector @@ plainto_tsquery('simple',ledger_fts_text(:q)) order by v.recorded_at desc limit 5",db.scoped(actor,"q",title));}
 ...
 private List<Requests.ClaimInput> publicationClaims(ActorContext actor,JsonNode doc){Map<Integer,List<Requests.EvidenceLink>> links=new HashMap<>();for(JsonNode m:doc.path("evidenceMappings")){try{UUID id=UUID.fromString(m.path("evidenceRef").asText());ledger.evidence(actor,id);links.computeIfAbsent(m.path("claimIndex").asInt(),x->new ArrayList<>()).add(new Requests.EvidenceLink(id,m.path("relation").asText("CONTEXT")));}catch(Exception ignored){}}
  List<Requests.ClaimInput> result=new ArrayList<>();int i=0;for(JsonNode c:doc.path("claims")){List<Requests.EvidenceLink> evidence=links.getOrDefault(i,List.of());boolean observed=c.path("kind").asText().equals("OBSERVED")&&evidence.stream().anyMatch(x->x.supportType().equals("SUPPORTS"));String origin=observed?"OBSERVED":c.path("originType").asText("AGENT_DERIVED");if(!Set.of("HUMAN_ASSERTED","AGENT_DERIVED").contains(origin)&&!observed)origin="AGENT_DERIVED";String type=observed?"OBSERVATION":c.path("claimType").asText("RECOMMENDATION");String method=origin.equals("AGENT_DERIVED")?firstNonBlank(c.path("derivationMethod").asText(),"LLM extraction from immutable candidate input"):null;result.add(new Requests.ClaimInput(type,c.path("content").asText(),origin,method,evidence));i++;}
  if(result.isEmpty())result.add(new Requests.ClaimInput("LESSON",first(doc,"reusablePrinciple","lesson","decision"),"AGENT_DERIVED","Draft synthesis from immutable candidate input",List.of()));return result;}
