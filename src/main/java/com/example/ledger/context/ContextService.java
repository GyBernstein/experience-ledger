...
public class ContextService {
 ...
 public Object context(ActorContext a,Context r){long start=System.nanoTime();return db.snapshot(a,"supply context",()->{
  ...
  var ids=db.list("""
   select v.id,ts_rank_cd(v.search_vector,plainto_tsquery('simple',ledger_fts_text(:query)),32) as rank,
    (select e.group_id from exp_problem_group_event e where e.space_id=v.space_id and e.family_id=v.family_id
      order by e.created_at desc,e.id desc limit 1) as problem_group_id,
    (select e.action from exp_problem_group_event e where e.space_id=v.space_id and e.family_id=v.family_id
      order by e.created_at desc,e.id desc limit 1) as group_action
  ...
  for(var row:ids){var s=load(a,(UUID)row.get("id"),loaded);baseline+=ContextBudget.units(db.stringify(s));if(!eligible(s,r.domain(),ctx,rules,confidence,needEvidence)){rejected++;continue;}
   if("LINK".equals(row.get("group_action")))s.put("problemGroupId",row.get("problem_group_id"));
