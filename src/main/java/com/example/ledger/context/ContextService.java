...
public class ContextService {
 ...
 private boolean eligible(Map<String,Object> s,String domain,JsonNode ctx,Rules rules,double confidence,boolean evidenceRequired){
  if(!store.live(s)||!domain.equals(s.get("domain"))||ContextStore.number(s,"assessed_confidence")<confidence)return false;
  ...
  var usable=ContextStore.rows(s.get("evidence")).stream().filter(e->ContextStore.number(e,"reliability")>=rules.minEvidenceReliability()).toList();
 ...
 private Supply supply(Map<String,Object> compact,List<Map<String,Object>> sources,Map<String,Object> rep,double relevance,int evidenceLimit,boolean needEvidence,String cached){
  ...
  meta.put("key",key);meta.put("compactId",compact==null?null:compact.get("id"));meta.put("representativeVersionId",rep.get("id"));meta.put("versionIds",versions);meta.put("evidence",evidence);meta.put("negative",ContextStore.negative(rep));meta.put("assessedConfidence",sources.stream().mapToDouble(s->ContextStore.number(s,"assessed_confidence")).min().orElse(0));meta.put("units",ContextBudget.units(text));meta.put("title",compact==null?rep.get("title"):compact.get("title"));
  return new Supply(new ContextBudget.Item(key,text,ContextStore.negative(rep),chosen,versions,relevance),meta);
 }
 private String rawText(Map<String,Object> s){return "EXPERIENCE "+s.get("id")+" ["+(ContextStore.negative(s)?"NEGATIVE":"POSITIVE")+"]\n"+s.get("title")+"\nLesson: "+s.get("lesson")+"\nDecision: "+s.get("decision")+"\nAction: "+s.get("action")+"\nScope: "+s.get("applicability_json")+"\nConstraints: "+s.get("constraints_json")+"\nClaims with original provenance: "+db.stringify(s.get("claims"))+"\nValidation: "+s.get("validation_status")+"; assessedConfidence="+s.get("assessed_confidence");}
